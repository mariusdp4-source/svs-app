...
class HORestockOrderHandler(BaseHandler):
    @tornado.web.authenticated
    def get(self):
        u = self.current_user
        if u['role'] != 'ho_admin':
        ...
        products = [dict(p) for p in db.execute(
            "SELECT * FROM products WHERE active=1 ORDER BY supplier, name"
        ).fetchall()]
        soh_map = {r['product_name']: r['quantity'] for r in db.execute(
            "SELECT product_name, quantity FROM storeroom_stock"
        ).fetchall()}
        outstanding_map = {r['product_name']: r['total'] for r in db.execute("""
        ...
        suppliers = {}
        for p in products:
            sup = p['supplier'] or 'Unknown'
            soh = int(soh_map.get(p['name'], 0) or 0)
            outstanding = int(outstanding_map.get(p['name'], 0) or 0)
            min_amt = int(p.get('min_stock') or 0)
            to_order = max(0, min_amt + outstanding - soh)
            if sup not in suppliers:
                suppliers[sup] = []
            suppliers[sup].append({
                'name': p['name'], 'category': p['category'],
                'soh': soh, 'min_amt': min_amt,
                'outstanding': outstanding, 'to_order': to_order, 'id': p['id'],
            })
        supplier_list = [[s, items] for s, items in sorted(suppliers.items())]
        db.close()
        self.render('ho/restock_order.html', user=u, supplier_list=supplier_list)


class HORestockWordHandler(BaseHandler):
    """Restock order as Word dokument."""
    @tornado.web.authenticated
    def get(self):
        u = self.current_user
        if u['role'] != 'ho_admin':
        ...
        try:
            ...
            self.write('python-docx nie geinstalleer nie'); return
        ...
        for p in products:
            ...
            suppliers[sup].append({
                'name': p['name'], 'category': p['category'],
                'soh': soh, 'min_amt': min_amt,
                'outstanding': outstanding, 'to_order': to_order,
            })
        ...
        run = title.add_run('RESTOCK ORDER')
        run.bold = True
        run.font.size = Pt(16)
        run.font.color.rgb = RGBColor(0x3D, 0x1F, 0x5C)

        date_p = doc.add_paragraph()
        date_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        date_p.add_run(datetime.date.today().strftime('%d %B %Y')).font.size = Pt(10)
        doc.add_paragraph()

        purple = RGBColor(0x3D, 0x1F, 0x5C)
        for sup, items in sorted(suppliers.items()):
            # Verskaffer opskrif
            h = doc.add_paragraph()
            r = h.add_run(sup.upper())
            r.bold = True
            r.font.size = Pt(12)
            ...
            for i, txt in enumerate(['Product', 'Stock on Hand', 'Min Amount', 'Outstanding', 'To Order']):
                hdr[i].text = txt
                hdr[i].paragraphs[0].runs[0].bold = True
                hdr[i].paragraphs[0].runs[0].font.size = Pt(9)
                hdr[i].paragraphs[0].runs[0].font.color.rgb = purple

            for item in items:
                row = tbl.add_row().cells
                row[0].text = item['name']
                row[1].text = str(item['soh'])
                row[2].text = str(item['min_amt'])
                row[3].text = str(item['outstanding']) if item['outstanding'] else '-'
                row[4].text = str(item['to_order'])
        ...
        doc.save(buf)
        buf.seek(0)
        fname = f"restock_order_{datetime.date.today().isoformat()}.docx"
...
def make_app():
    ...
    return tornado.web.Application(
        [
            ...
            (r'/ho/restock', HORestockOrderHandler),
            (r'/ho/restock/word', HORestockWordHandler),
